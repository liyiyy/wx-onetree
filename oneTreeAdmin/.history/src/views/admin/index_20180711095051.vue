    ...
    <el-row :gutter="20" style="width:95%">
     ...
     <el-table
      ...
      <el-table-column
        ...
        label="操作">
        <template slot-scope="scope">
          <el-button type="warning" @click="forbid(scope.row)">禁用</el-button>
...
import { adminRegist, getList, delAdmin } from '@/api/adminMg'
...
export default {
  ...
  methods: {
    ...
    forbid(row) {
      forbid({ id: row._id, forbid: row.forbid }).then(res => {
        if (res.code === 0) {
          this.fetchData()
        }
      })
