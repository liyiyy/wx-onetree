...
router.post('/upload', async(ctx) => {
  ...
  consol / e.log(data)
  var mathname = (Math.random() * 1000 + 1) + '.jpg'

  const savePath = path.join(`./files`, mathname)
  const reader = fs.createReadStream(data.data)
  ...
  const pro = new Promise((resolve, reject) => {
    ...
    stream.on('finish', function() {
      resolve(`http://localhost:1122${mathname}`)
    })
  })
